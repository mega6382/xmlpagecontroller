...
abstract class XMLTagParser
{
	private $m_tagName = "unknown";

	public function __construct($a_tagName = "unknown" )
	{
		$this->m_tagName = $a_tagName;
	}
...
*/
	...
	public function addPlugin(&$obj )
	{
		...
		{
			return;
		}/*if( isset($this->plugins[ $obj->name() ]) == false ) $this->plugins[ $obj->name() ] = $obj;
else */
		$this->log("Register plugin: " . $obj->name() );
...
*/
	private function parseRecursive(XMLTag & $node, $parent = null )
	{
		...
		{//echo '&nbsp;&nbsp;&nbsp;&nbsp;Parsing child: "'. $child->name() . '"<br />';
...
*/
	private function parseSwitch (XMLTag & $node, &$parent )
	{
		...
		switch ($node->name() )
		{
			default: $this->log('Bad switch data ', 1 );
			return;
			break;
		case 'switch':
		case 'switch:get':
		...
		{//$this->log( 'Switch data is: ' .serialize($switchData), 1 );
		}
		...
		{//$this->log( $case->attr('name') . ' and ' . $switchValue, 1 );
			switch ($case->name() )
			{ case 'default':
				{
					$default = $case;
	...
	private function parseForeach (XMLTag & $node )
	{
		...
		switch ($node->name() )
		{
			default: $this->log('Bad foreach data ', 1 );
			return;
			break;
		case 'foreach':
		case 'foreach:get':
...
*/
	private function parseNode(XMLTag & $node, $parent)
	{
		...
		function_exists('xmlpage_parser_'.$name) )
		{
		...
		case 'locale':
			{
				...
				if (!is_string($attrLang) || strlen($attrLang) == 0 || $attrLang != $this->lang )
				{
					break;
				}
...
*/
	function __construct($options = array() )
	{
		...
		$out = false;
		$this->debug = true;
		$container = "";//$this->log('Construct XMLPAGE');
...
*/
	...
	public function addJS(&$content )
	{
		...
		$this->log('Inline javascript len: '. strlen($content) );//echo "HEELO SCRIPT inline <br />";
	}
...
*/
	...
	private function popContent()
	{
		...
		$this->logstack--;//$this->log( "Pop content" );
	}
}
?>